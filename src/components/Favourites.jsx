      ...
      <div className="flex justify-center gap-2 mt-2">
        {filmPosters.map((_, index) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition-all duration-300 ${Math.floor(currentIndex) === index
                ? 'bg-[#D14538] w-4'
        ...
        {allItems.map((item, index) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4">
              <div className="text-[14px] uppercase tracking-[0.14em] font-bold text-amber-100">
                {item.label}
              </div>
            </div>
          </div>
      ...
      >
        {allItems.map((item, index) => (
          ...
          >
            ...
            {/* Content */}
            <div className="absolute bottom-0 left-0 right-0 p-4">
              <div className="text-[13px] uppercase tracking-[0.14em] font-bold mb-1 text-amber-100 line-clamp-2">
                {item.label}
              </div>
              <div className="text-[11px] text-amber-200/80 font-medium">{item.note}</div>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {categories.map(cat => (
              <button
                ...
                className={`text-[13px] uppercase tracking-[0.18em] px-5 py-2.5 rounded-full border-2 transition-all duration-200 cursor-pointer font-bold ${activeCategory === cat.id
                    ? "bg-[#D14538] text-white border-[#D14538] shadow-[0_4px_0_rgba(0,0,0,0.2)] -translate-y-0.5"
            ...
            {activeCategory === "films" ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {activeData?.items?.map((item, index) => (
                  <div
                    key={item.label}
                    className={`relative aspect-[3/4] rounded-[16px] bg-gradient-to-br from-[#3d3020] to-[#2a2218] shadow-[0_16px_0_rgba(0,0,0,0.3)] overflow-hidden transition-all duration-300 group border border-amber-500/20 ${index % 2 === 0 ? "-rotate-1" : "rotate-1"
